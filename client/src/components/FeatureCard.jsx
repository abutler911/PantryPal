...
function FeatureCard() {
  ...
  return (
    <section className="dark:bg-gray-900 bg-white px-4 py-12 mx-auto max-w-7xl">
      <h1 className="text-4xl font-bold text-center mb-8 dark:text-gray-100 text-gray-900">
        Features
      </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {cardDetails.map((card) => (
          ...
          >
            <div className="text-brand-green dark:text-brand-green text-5xl mb-4">
              {card.icon}
            </div>
            <h2 className="text-xl font-semibold mb-2 dark:text-gray-100 text-gray-900">
              {card.title}
            </h2>
            <p className="prose prose-sm dark:prose-invert text-gray-600 dark:text-gray-300">
              {card.description}
            </p>
          </div>
        ))}
